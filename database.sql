...
-- Tabla Usuarios
-- CORRECCIÓN #11: Se agregó 'Administrativo' al ENUM de rol

CREATE TABLE IF NOT EXISTS Usuarios (
    id_usuario     INT AUTO_INCREMENT PRIMARY KEY,
    nombre         VARCHAR(100)  NOT NULL,
    email          VARCHAR(100)  UNIQUE NOT NULL,
    rol            ENUM('Administrador', 'Estudiante', 'Docente', 'Administrativo') NOT NULL,
    contrasena     VARCHAR(255)  NOT NULL,
    fecha_creacion TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS Administrativos (
    ...
    CONSTRAINT fk_administrativo_usuario
        FOREIGN KEY (id_administrativo) REFERENCES Usuarios(id_usuario)
        ON DELETE CASCADE ON UPDATE CASCADE
);
...
-- Tabla Comentarios
-- CORRECCIÓN #5:  promedio cambiado de INT a DECIMAL(3,1)
-- CORRECCIÓN #10: se agrega columna id_evaluacion (referenciada en procedimientos y vistas)
CREATE TABLE IF NOT EXISTS Comentarios (
    id_comentario  INT AUTO_INCREMENT PRIMARY KEY,
    id_estudiante  INT,
    id_docente     INT,
    id_asignatura  INT,
    id_evaluacion  INT,
    promedio       DECIMAL(3,1) CHECK (promedio BETWEEN 1 AND 5),
    comentario     TEXT NOT NULL,
    sentimiento    ENUM('positivo', 'negativo', 'neutral'),
    fecha_creacion TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_comentario_estudiante
        FOREIGN KEY (id_estudiante)  REFERENCES Usuarios(id_usuario),
    CONSTRAINT fk_comentario_docente
        FOREIGN KEY (id_docente)     REFERENCES Usuarios(id_usuario),
    CONSTRAINT fk_comentario_asignatura
        FOREIGN KEY (id_asignatura)  REFERENCES Asignaturas(id_asignatura),
    CONSTRAINT fk_comentario_evaluacion
...
CREATE TABLE IF NOT EXISTS Analisis_sentimientos (
    ...
    resumen      TEXT,
    puntuacion   DECIMAL(3,2),
    CONSTRAINT fk_analisis_comentario
...
-- Tabla MDS (Materia-Docente-Semestre)
CREATE TABLE IF NOT EXISTS MDS (
    id_mds        INT AUTO_INCREMENT PRIMARY KEY,
    id_docente    INT,
    id_asignatura INT,
    id_semestre   INT,
    CONSTRAINT fk_mds_docente
        FOREIGN KEY (id_docente)    REFERENCES Docente(id_docente),
    CONSTRAINT fk_mds_asignatura
        FOREIGN KEY (id_asignatura) REFERENCES Asignaturas(id_asignatura),
    CONSTRAINT fk_mds_semestre
        FOREIGN KEY (id_semestre)   REFERENCES Semestre(id_semestre)
);

-- Tabla ME (Materia-Estudiante)
CREATE TABLE IF NOT EXISTS ME (
    id_me         INT AUTO_INCREMENT PRIMARY KEY,
    id_estudiante INT,
    id_asignatura INT,
    CONSTRAINT fk_me_estudiante
        FOREIGN KEY (id_estudiante) REFERENCES Estudiante(id_estudiante),
    CONSTRAINT fk_me_asignatura
        FOREIGN KEY (id_asignatura) REFERENCES Asignaturas(id_asignatura)
);
...
--  1 Fernando López  y demás admins  → Administrador (rol corregido a 'Administrativo')

-- CORRECCIÓN #1-3: roles de IDs 9-13 cambiados a 'Administrativo'
--   para que puedan insertarse correctamente en la tabla Administrativos.
...
('Laura Pérez',     'laura.perez@uniautonoma.edu.co',       'Docente',        '123'),
('Miguel Soto',     'miguel.soto@uniautonoma.edu.co',       'Docente',        '123'),
('Elena Ruiz',      'elena.ruiz@uniautonoma.edu.co',        'Administrativo',  '123'),
('Ana María',       'anamaria@uniautonoma.edu.co',          'Docente',        '123'),
('Diego Prado',     'diegoprado@uniautonoma.edu.co',        'Docente',        '123'),
('Christian Cañar', 'christiancanar@uniautonoma.edu.co',    'Docente',        '123'),
('Andrés Torres',   'andres.torres@uniautonoma.edu.co',     'Administrativo', 'admin123'),
('Sofía Martínez',  'sofia.martinez@uniautonoma.edu.co',    'Administrativo', 'admin123'),
('Luis Ramírez',    'luis.ramirez@uniautonoma.edu.co',      'Administrativo', 'admin123'),
('Camila Herrera',  'camila.herrera@uniautonoma.edu.co',    'Administrativo', 'admin123'),
('Fernando López',  'fernando.lopez@uniautonoma.edu.co',    'Administrativo', 'admin123');


-- CORRECCIÓN #1: IDs y títulos ahora son coherentes con los usuarios insertados
INSERT INTO Docente (id_docente, titulo, certificado) VALUES
(3, 'Magíster en Ciencias de la Computación', 'certificados/laura_perez.pdf'),
(4, 'Licenciado en Matemáticas',              'certificados/miguel_soto.pdf'),
(6, 'Ingeniera de Sistemas',                  'certificados/ana_maria.pdf'),
(7, 'Doctor en Educación',                    'certificados/diego_prado.pdf'),
(8, 'Ingeniero en Informática',               'certificados/christian_canar.pdf');

-- Estudiantes (IDs 1 y 2 — únicos con rol 'Estudiante')
-- CORRECCIÓN #2: eliminados IDs 9, 10, 11 (son Administrativos).
--   Se agregan 3 nuevos usuarios Estudiante si se necesitan más registros.
INSERT INTO Estudiante (id_estudiante, estado, codigo, telefono, cuenta_social) VALUES
(1, 'Activo',   'EST1001', '555-1234', '@nicolasvargas'),
(2, 'Activo',   'EST1002', '555-5678', '@carlosm');

-- Administrativos (IDs 9-13 — todos con rol 'Administrativo')
-- CORRECCIÓN #3: ya no hay conflicto con la tabla Estudiante
INSERT INTO Administrativos (id_administrativo, area, cargo, telefono, correo_institucional) VALUES
(5,  'Recursos Humanos', 'Coordinadora',   '555-2468', 'elena.ruiz@uni.edu'),
(9,  'Admisiones',       'Asistente',      '555-9753', 'andres.torres@uni.edu'),
(10, 'Contabilidad',     'Analista',       '555-8642', 'sofia.martinez@uni.edu'),
(11, 'Tecnología',       'Soporte Técnico','555-7531', 'luis.ramirez@uni.edu'),
(12, 'Biblioteca',       'Administrativo',  '555-1597', 'camila.herrera@uni.edu');

-- Asignaturas
-- CORRECCIÓN #4: id_docente apunta solo a IDs que existen en la tabla Docente (3,4,6,7,8)
INSERT INTO Asignaturas (nombre_asignatura, creditos, id_docente) VALUES
...
-- Evaluaciones
INSERT INTO Evaluaciones (fecha_inicio, fecha_fin, estado, descripcion) VALUES
('2025-01-01', '2025-01-31', 'Inactivo', 'Evaluación inicial del semestre.'),
('2025-04-01', '2025-04-30', 'Activo',   'Evaluación de mitad de semestre.'),
('2025-07-01', '2025-07-31', 'Inactivo', 'Evaluación de seguimiento.'),
('2025-10-01', '2025-10-31', 'Inactivo', 'Evaluación final.'),
...
-- Comentarios
-- CORRECCIÓN #5: promedio ahora es DECIMAL(3,1), admite valores como 4.5
-- CORRECCIÓN #6: id_docente apunta a IDs en tabla Docente; id_estudiante a tabla Estudiante
-- CORRECCIÓN #10: se incluye id_evaluacion
INSERT INTO Comentarios (id_estudiante, id_docente, id_asignatura, id_evaluacion, comentario, sentimiento, promedio) VALUES
(1, 3, 1, 1, 'Muy buena metodología.',             'positivo', 4.5),
(2, 4, 2, 2, 'El docente explica con claridad.',   'positivo', 4.0),
(1, 6, 3, 3, 'A veces las clases son confusas.',   'negativo', 2.8),
(2, 7, 4, 4, 'Excelente manejo del tema.',         'positivo', 4.9),
(1, 8, 5, 5, 'Podría mejorar la interacción.',     'neutral',  3.2);

-- Análisis de Sentimientos
INSERT INTO Analisis_sentimientos (id_comentario, sentimiento, resumen, puntuacion) VALUES
(1, 'Positivo', 'Comentario favorable sobre la metodología.', 0.95),
(2, 'Positivo', 'Explicaciones claras.',                      0.90),
(3, 'Negativo', 'Confusión en algunas clases.',               0.30),
(4, 'Positivo', 'Excelente manejo.',                          0.97),
...
-- Reportes
INSERT INTO Reportes (id_docente, contenido, formato) VALUES
(3, 'Reporte del primer trimestre.',        'PDF'),
(4, 'Reporte del segundo trimestre.',       'Excel'),
(6, 'Reporte de evaluación especial.',      'PDF'),
(7, 'Reporte de actividades académicas.',   'Excel'),
(8, 'Reporte final del semestre.',          'PDF');

-- Semestre
INSERT INTO Semestre (nombre_semestre, periodo, fecha_inicio, fecha_final) VALUES
('Semestre 2024-1', 2024, '2024-01-01', '2024-06-30'),
('Semestre 2024-2', 2024, '2024-07-01', '2024-12-31'),
('Semestre 2025-1', 2025, '2025-01-01', '2025-06-30'),
('Semestre 2025-2', 2025, '2025-07-01', '2025-12-31'),
...
-- Programa
INSERT INTO Programa (nombre_programa, codigo) VALUES
('Ingeniería de Sistemas',     'IS2025'),
('Ciencias de la Computación', 'CC2025'),
('Matemáticas Aplicadas',      'MA2025'),
('Administración de Empresas', 'AE2025'),
...
-- MDS (Materia-Docente-Semestre)
INSERT INTO MDS (id_docente, id_asignatura, id_semestre) VALUES
(3, 1, 1),
(4, 2, 2),
(8, 3, 3),
(6, 4, 4),
...
-- ME (Materia-Estudiante)
INSERT INTO ME (id_estudiante, id_asignatura) VALUES
(1, 1),
(2, 2),
(1, 3),
(2, 4),
...
BEGIN
    ...
    WHERE me.id_estudiante = p_id_estudiante;
END$$
DELIMITER ;

-- Validar Login (obtener usuario por email)
DELIMITER $$
...
DELIMITER $$
CREATE PROCEDURE LeerUsuario(IN p_id INT)
BEGIN
    SELECT id_usuario, nombre, email, rol
    FROM Usuarios
    WHERE id_usuario = p_id;
...
BEGIN
    DECLARE msg TEXT;
    IF EXISTS (SELECT 1 FROM Comentarios WHERE id_estudiante = p_id) THEN
        SET msg = CONCAT('No se puede eliminar el usuario con ID ', p_id,
                         ' porque tiene comentarios relacionados.');
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = msg;
    ELSE
        DELETE FROM Usuarios WHERE id_usuario = p_id;
    END IF;
...
DELIMITER $$
CREATE PROCEDURE LeerAsignatura(IN p_id INT)
BEGIN
    SELECT id_asignatura, nombre_asignatura, id_docente
    FROM Asignaturas
    WHERE id_asignatura = p_id;
...
DELIMITER $$
CREATE PROCEDURE EliminarAsignatura(IN p_id INT)
BEGIN
    DELETE FROM Asignaturas WHERE id_asignatura = p_id;
END$$
DELIMITER ;
...
DELIMITER $$
CREATE PROCEDURE LeerEvaluacion(IN p_id INT)
BEGIN
    SELECT id_evaluacion, fecha_inicio, fecha_fin, estado, descripcion
    FROM Evaluaciones
    WHERE id_evaluacion = p_id;
...
DELIMITER $$
CREATE PROCEDURE EliminarEvaluacion(IN p_id INT)
BEGIN
    DELETE FROM Evaluaciones WHERE id_evaluacion = p_id;
END$$
DELIMITER ;
...
CREATE PROCEDURE InsertarComentario(
    ...
    IN p_comentario    TEXT,
    IN p_sentimiento   TEXT,
    IN p_promedio      DECIMAL(3,1)
)
BEGIN
    INSERT INTO Comentarios
        (id_estudiante, id_docente, id_asignatura, comentario, sentimiento, promedio)
    VALUES
        (p_id_estudiante, p_id_docente, p_id_asignatura,
         p_comentario, p_sentimiento, p_promedio);
END$$
DELIMITER ;

-- CORRECCIÓN #10: se incluye id_evaluacion en el SELECT
DELIMITER $$
CREATE PROCEDURE LeerComentario(IN p_id INT)
BEGIN
    SELECT
        id_comentario,
        id_estudiante,
...
DELIMITER $$
CREATE PROCEDURE EliminarComentario(IN p_id INT)
BEGIN
    DELETE FROM Comentarios WHERE id_comentario = p_id;
END$$
DELIMITER ;

-- Resumen de sentimientos global
-- CORRECCIÓN #9: DELIMITER ; con espacio correcto
DELIMITER $$
CREATE PROCEDURE resumen_sentimientos_global()
BEGIN
    DECLARE total_count INT;
...
DELIMITER $$
CREATE PROCEDURE usuarios_sin_contrasena()
BEGIN
    SELECT id_usuario, nombre, email, rol, fecha_creacion FROM Usuarios;
END$$
DELIMITER ;
...
-- Resumen de sentimientos por nombre de docente
DELIMITER $$
CREATE PROCEDURE resumen_sentimientos_por_nombre_docente(
    IN nombre_docente_input VARCHAR(255)
)
BEGIN
    SELECT
        d.id_usuario AS id_docente,
...
-- Consulta de sentimientos como porcentaje (era CTE suelta sin FROM)
-- CORRECCIÓN #7: convertida en procedimiento completo y correcto
DELIMITER $$
CREATE PROCEDURE resumen_porcentaje_sentimientos()
BEGIN
    SELECT
        LOWER(sentimiento)                       AS sentimiento,
        COUNT(*)                                 AS total,
...
-- Nota: la restricción UNIQUE ya cubre esto; el trigger queda
--       como capa de mensaje personalizado.
DELIMITER $$
CREATE TRIGGER trigger_validar_email
BEFORE INSERT ON Usuarios
FOR EACH ROW
BEGIN
    ...
    DECLARE msg VARCHAR(255);
    SELECT COUNT(*) INTO email_duplicado
    FROM Usuarios WHERE email = NEW.email;
    IF email_duplicado > 0 THEN
        SET msg = CONCAT('El correo electrónico ', NEW.email, ' ya está registrado.');
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = msg;
    END IF;
...
-- Solo correos institucionales — INSERT
DELIMITER $$
CREATE TRIGGER validar_email_institucional
BEFORE INSERT ON Usuarios
FOR EACH ROW
BEGIN
    IF RIGHT(LOWER(TRIM(NEW.email)), LENGTH('@uniautonoma.edu.co'))
           != '@uniautonoma.edu.co' THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Solo se permiten correos institucionales (@uniautonoma.edu.co)';
    END IF;
END$$
...
-- Solo correos institucionales — UPDATE
DELIMITER $$
CREATE TRIGGER validar_email_institucional_update
BEFORE UPDATE ON Usuarios
FOR EACH ROW
...
-- Proteger administradores de borrado
DELIMITER $$
CREATE TRIGGER trigger_proteger_admin
BEFORE DELETE ON Usuarios
FOR EACH ROW
...
-- Proteger cambio de rol de administradores
DELIMITER $$
CREATE TRIGGER trigger_proteger_admin_actualizacion
BEFORE UPDATE ON Usuarios
FOR EACH ROW
...
-- Validar campos obligatorios al insertar usuario
DELIMITER $$
CREATE TRIGGER trigger_validar_usuario_insert
BEFORE INSERT ON Usuarios
FOR EACH ROW
...
-- Validar campos obligatorios al actualizar usuario
DELIMITER $$
CREATE TRIGGER trigger_validar_usuario_update
BEFORE UPDATE ON Usuarios
FOR EACH ROW
...
-- Evitar comentarios duplicados (mismo estudiante, docente y asignatura)
DROP TRIGGER IF EXISTS validar_comentario;
DELIMITER $$
CREATE TRIGGER validar_comentario
BEFORE INSERT ON Comentarios
...
-- Proteger docente con asignaturas asignadas
-- CORRECCIÓN #8: DELIMITER $$ en lugar de $ (un solo símbolo)
DELIMITER $$
CREATE TRIGGER proteger_docente
BEFORE DELETE ON Docente
FOR EACH ROW
BEGIN
    IF EXISTS (SELECT 1 FROM Asignaturas WHERE id_docente = OLD.id_docente) THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'No se puede eliminar el docente porque tiene asignaturas asignadas.';
    END IF;
END$$
DELIMITER ;

-- Proteger estudiante con comentarios registrados
DELIMITER $$
CREATE TRIGGER trg_no_eliminar_estudiante_con_comentarios
BEFORE DELETE ON Estudiante
FOR EACH ROW
BEGIN
    IF EXISTS (SELECT 1 FROM Comentarios WHERE id_estudiante = OLD.id_estudiante) THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'No se puede eliminar el estudiante porque tiene comentarios registrados.';
    END IF;
END$$
DELIMITER ;

-- Validar rol sin espacios — INSERT
DELIMITER $$
CREATE TRIGGER validar_rol_sin_espacios
BEFORE INSERT ON Usuarios
FOR EACH ROW
...
-- Validar rol sin espacios — UPDATE
DELIMITER $$
CREATE TRIGGER validar_rol_sin_espacios_update
BEFORE UPDATE ON Usuarios
FOR EACH ROW
...
-- Evitar descripción duplicada en Evaluaciones — INSERT
DELIMITER $$
CREATE TRIGGER evitar_descripciones_duplicadas
BEFORE INSERT ON Evaluaciones
FOR EACH ROW
BEGIN
    IF EXISTS (SELECT 1 FROM Evaluaciones WHERE descripcion = NEW.descripcion) THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Ya existe una evaluación con esta descripción.';
    END IF;
END$$
...
-- Evitar descripción duplicada en Evaluaciones — UPDATE
-- CORRECCIÓN #9: DELIMITER al final corregido (tenía // sin cierre de DELIMITER ;)
DELIMITER $$
CREATE TRIGGER evitar_descripciones_duplicadas_update
BEFORE UPDATE ON Evaluaciones
FOR EACH ROW
...
-- Evitar campos nulos en comentarios — INSERT
DELIMITER $$
CREATE TRIGGER evitar_campos_nulos_comentarios_insert
BEFORE INSERT ON Comentarios
FOR EACH ROW
...
-- Evitar campos nulos en comentarios — UPDATE
DELIMITER $$
CREATE TRIGGER evitar_campos_nulos_comentarios_update
BEFORE UPDATE ON Comentarios
FOR EACH ROW
